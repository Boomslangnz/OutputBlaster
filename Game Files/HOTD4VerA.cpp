...
{
	HOTD4BasePtr = helpers->ReadIntPtr(cgGLBase + 0x9164, false);
	HOTD4PtrA = helpers->ReadIntPtr(HOTD4BasePtr + 0x11C, false);
	HOTD4PtrB = helpers->ReadIntPtr(HOTD4PtrA + 0x50, false);
	HOTD4Ptr1P = helpers->ReadIntPtr(HOTD4PtrB + 0x20, false);
	HOTD4Ptr2P = helpers->ReadIntPtr(HOTD4PtrB + 0x38, false);

	JvsBaseA = helpers->ReadIntPtr(cgGLBase + 0x9164, false);
	JvsBaseB = helpers->ReadIntPtr(JvsBaseA + 0x11C, false);
	JvsBaseC = helpers->ReadIntPtr(JvsBaseB + 0x50, false);
	JvsBaseD = helpers->ReadIntPtr(JvsBaseC + 0x20, false);
	JvsBaseE = helpers->ReadIntPtr(JvsBaseC + 0x38, false);

	InGame = helpers->ReadByte(0xA71179C, false);

	JvsP1State = (HOTD4_PLAYER_STATE)helpers->ReadByte(JvsBaseD + 0x38, false);
	JvsP2State = (HOTD4_PLAYER_STATE)helpers->ReadByte(JvsBaseE + 0x38, false);
...
void HOTD4VerA::OutputsGameLoop()
{
	if (!init)
	{
		cgGLBase = (uintptr_t)GetModuleHandleA("cgGL.dll");
		Outputs = CreateOutputsFromConfig();
		m_game.name = "House Of The Dead 4 (Version A)";

...
void CNetOutputs::SendOutput(EOutputs output, UINT8 prevValue, UINT8 value)
{
	...
	for (vector<RegisteredClientTcp>::iterator it = m_clients.begin(), end = m_clients.end(); it != end; it++) {
		...
		std::string name = MapIdToName(output) ? MapIdToName(output) : "";
...
LRESULT CNetOutputs::SendStarIdtString(RegisteredClientTcp& client)
{
...
LRESULT CNetOutputs::SendIdString(RegisteredClientTcp &client, LPARAM id)
{
	// Id 0 is the name of the game
	std::string name;
	if (id == 0)
...
const char * CNetOutputs::MapIdToName(LPARAM id)
{
	EOutputs output = (EOutputs)(id - 1);
	return GetOutputName(output);
}

LPARAM CNetOutputs::MapNameToId(const char *name)
{
